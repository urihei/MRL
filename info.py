...
class AgentInfo(object):
    ...
    def update(self, action: ActionType, events: list[str], properties: dict[str, float]):
        ...
        for agent_property, property_value in properties.items():
            self.properties_list.setdefault(agent_property, [0.0] * self.current_step).append(property_value)
        self.current_step += 1

    def get_dict(self):
        d = {'actions': self.actions[-1], 'events': self.event_list[-1]}
        d.update({k.capitalize(): p_l[-1] for k, p_l in self.properties_list.items()})
        return d
...
class Info(object):
    ...
    def info_current(self):
        return "\n".join([a.info_current() for a in self.agent_info_object.values()])
